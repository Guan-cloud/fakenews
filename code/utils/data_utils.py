    ...
    def create_examples(self, datas, set_type):
        ...
        for i, data in enumerate(datas):
            ...
            if set_type == 'test':
                ...
                if str(data[1]) == 'nan' or str(data[1]) == '':
                    label='0'
                else:
                    label = str(int(data[1]))
            examples.append(
                InputExample(
                    guid=guid,
                    text_a=text_a,
...
def multi_classification_convert_examples_to_dataset(
    ...
    all_attention_masks = torch.tensor([f[0].attention_mask for f in features], dtype=torch.long)
    all_token_type_ids = torch.tensor([f[0].token_type_ids for f in features], dtype=torch.long) 
    if set_type != 'test':
        all_labels = torch.tensor([f[0].label for f in features], dtype=torch.long)
...
def compute_metrics(y_true, y_pred, average):
    ...
    weight_f1 = f1_score(y_true, y_pred, average=average)
