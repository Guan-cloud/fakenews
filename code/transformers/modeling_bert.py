...
# BertAttention可以拆分BertSelfAttention和BertSelfOutput两部分
...
class BertLayer(nn.Module):
    # 通过BertEncoder代码能够看出，BertEncoder是由多层BertLayer叠加而成，我们把BertLayer这个结构单独拿出来，
    def __init__(self, config):
        super().__init__()
    ...
    # BertAttention，传参进去的mask后续会有详细解释
        ...
        # 这段代码里的chunking部分不是给BERT用的，但是源码把BertIntermediate、BertOutput都封装到里面了，
        # 所以我们直接看feed_forward_chunk这个函数就可以了
...
class BertEncoder(nn.Module):  # 很多个transformer块
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear((self.num_filters_ * len(self.filter_sizes_)) * 4 + config.hidden_size,
                                    config.num_labels)
    ...
    def forward(
        ...
        sequence_output1 = outputs[2][-1]
        out1 = sequence_output1.unsqueeze(1)
        out1 = torch.cat([self.conv_and_pool(out1, conv) for conv in self.convs], 1)

        sequence_output2 = outputs[2][-2]
        out2 = sequence_output2.unsqueeze(1)
        out2 = torch.cat([self.conv_and_pool(out2, conv) for conv in self.convs], 1)

        sequence_output3 = outputs[2][-3]
        out3 = sequence_output3.unsqueeze(1)
        out3 = torch.cat([self.conv_and_pool(out3, conv) for conv in self.convs], 1)

        sequence_output4 = outputs[2][-4]
        out4 = sequence_output4.unsqueeze(1)
        out4 = torch.cat([self.conv_and_pool(out4, conv) for conv in self.convs], 1)

        out = torch.cat((out1, out2, out3, out4, outputs[1]), 1)
...
class BertForSequenceClassificationoooo(BertPreTrainedModel):
    def __init__(self, config, args):
        super().__init__(config)
        self.num_labels = config.num_labels
